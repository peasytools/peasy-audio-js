...
/** PeasyAudio API client. Zero dependencies — uses native fetch. */
export class PeasyAudio {
  private baseUrl: string;

  constructor(baseUrl = "https://peasyaudio.com") {
  ...
  private async get<T>(path: string, params?: Record<string, string | number | undefined> | object): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`PeasyAudio API error: HTTP ${res.status} — ${body}`);
    }
